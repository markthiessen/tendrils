...
export function registerTaskRoutes(app: FastifyInstance, ctx: ServerContext) {
  app.get<{ Querystring: { activityId?: string } }>("/api/tasks", (req) => {
    ...
    return { ok: true, data: findAllTasks(ctx.db, actId) };
  });

  app.get<{ Params: { id: string } }>("/api/tasks/:id", (req) => {
    const t = findTaskById(ctx.db, Number(req.params.id));
  ...
  app.post<{ Body: { activityId: number; title: string; description?: string } }>("/api/tasks", (req) => {
    const t = insertTask(ctx.db, req.body.activityId, req.body.title, req.body.description ?? "");
    emit("task.created", t);
    return { ok: true, data: t };
  });

  app.put<{ Params: { id: string }; Body: { title?: string; description?: string } }>("/api/tasks/:id", (req) => {
    const t = updateTask(ctx.db, Number(req.params.id), req.body);
    if (!t) return { ok: false, error: { code: "NOT_FOUND", message: "Task not found" } };
    emit("task.updated", t);
    return { ok: true, data: t };
  });
  ...
  app.delete<{ Params: { id: string } }>("/api/tasks/:id", (req) => {
    ...
    const deleted = deleteTask(ctx.db, id);
