...
export function registerBugRoutes(app: FastifyInstance, ctx: ServerContext) {
  app.get<{ Querystring: { severity?: string; status?: string; claimedBy?: string } }>("/api/bugs", (req) => {
    return { ok: true, data: findAllBugs(ctx.db, { severity: req.query.severity, status: req.query.status, claimedBy: req.query.claimedBy }) };
  });

  app.get<{ Params: { id: string } }>("/api/bugs/:id", (req) => {
    const b = findBugById(ctx.db, Number(req.params.id));
  ...
  app.post<{ Body: { title: string; description?: string; severity?: BugSeverity; linkedStoryId?: number; linkedTaskId?: number } }>("/api/bugs", (req) => {
    const b = insertBug(ctx.db, req.body.title, req.body.description ?? "", {
  ...
  app.put<{ Params: { id: string }; Body: { title?: string; description?: string; severity?: BugSeverity } }>("/api/bugs/:id", (req) => {
    const b = updateBug(ctx.db, Number(req.params.id), req.body);
    if (!b) return { ok: false, error: { code: "NOT_FOUND", message: "Bug not found" } };
    emit("bug.updated", b);
    return { ok: true, data: b };
  });
  ...
  app.delete<{ Params: { id: string } }>("/api/bugs/:id", (req) => {
    ...
    const deleted = deleteBug(ctx.db, id);
  ...
  app.post<{ Params: { id: string }; Body: { agent?: string } }>("/api/bugs/:id/claim", (req) => {
    const id = Number(req.params.id);
    const bug = findBugById(ctx.db, id);
    ...
    ctx.db.prepare("UPDATE bugs SET status = 'claimed', claimed_by = ?, claimed_at = datetime('now'), updated_at = datetime('now') WHERE id = ?")
      .run(req.body.agent ?? null, id);
    insertLogEntry(ctx.db, "bug", id, `Claimed by ${req.body.agent ?? "unknown"}`, req.body.agent, bug.status, "claimed");

    const updated = findBugById(ctx.db, id)!;
  ...
  app.post<{ Params: { id: string }; Body: { status: string; reason?: string; agent?: string } }>("/api/bugs/:id/status", (req) => {
    ...
    values.push(id);
    ctx.db.prepare(`UPDATE bugs SET ${sets.join(", ")} WHERE id = ?`).run(...values);
    insertLogEntry(ctx.db, "bug", id, `Status -> ${newStatus}`, req.body.agent, bug.status, newStatus);
